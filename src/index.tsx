...
function startLoraTraining(folderPaths: FolderPaths) {
  ...
  const command = `. \"${venvPath}\" && CUDA_VISIBLE_DEVICES=${globalThis.advanceParams?.get('CUDA_VISIBLE_DEVICES')} accelerate launch --num_cpu_threads_per_process=2 "${scripPath}" --pretrained_model_name_or_path="${modelPath}" --train_data_dir="${dataDir}" --output_dir="${outputDir}" --logging_dir="${loggingDir}" --save_model_as=safetensors --output_name="${modelName}" --caption_extension=".txt" --sample_prompts="${samplePromptPath}" ${getCommandsParams()}` ;
